...
export class UsersService {
  constructor(@InjectModel('User') private readonly userModel: Model<User>, private bcryptService: BcryptService) { }
  ...
  async create(user: CreateUserDto) {
    var userExist = await this.getByEmail(user.email);
  ...
  async findByIdAndUpdate(id: string, user: UpdateUserDto) {
    try {
      ...
      if (exist) {
        return await this.userModel.findByIdAndUpdate(id, { $set: user }, { new: true }).exec();
      }
